...
.randomCellsLonLat <- function(r, n) {
  ...
  #skip since we are in projected***
  y <- yFromRow(r, 1:nrow(r)) # get y coordinates for all rows
  dx <- pointDistance(cbind(0, y), cbind(xres(r), y), longlat=F)  # size of each cell in longitude directoin
...
..randomCellsLonLat2 <- function(r, n) {
  ...
  dx <- pointDistance(cbind(0, y), cbind(xres(r), y), longlat=F)  
...
randomPoints_terra_sf <- function(mask, n, p, ext=NULL, extf=1.1, excludep=TRUE, prob=FALSE, cellnumbers=FALSE, tryf=3, warn=2, lonlatCorrection=TRUE) {
