...
#--------------------------------------------------
#Understand how common/rare species are in the study region
rapid_sp_sum<-rapid_dat%>%
...
if(!file.exists(paste0(path_out,"Final_outputs/Model_Results/plant_species_freq",".csv"))){
  write.csv(rapid_sp_sum,paste0(path_out,"Final_outputs/Model_Results/plant_species_freq",".csv"), row.names = F)
}

# Focus on species that can be used as a reference throughout the breeding range
...
rapid_dat2<-rapid_dat%>%
  ...
  ## Select Important Variables:
  dplyr::select(id, PatchID,Lat, Long, year,Date,SHARPTide,Time,data,ObserverInitials,
                #Categorical variables:
                ...
                DeadSnags,
                #Percent cover of dominant species (don't need to sum to 100%)
                # species names
  ...
  mutate(across(c(DomSp1,DomSp2,DomSp3,DomSp4,DomSp5,DomSp6,DomSp7,DomSp8,DomSp9,DomSp10),
                ...
                ~case_when(#alt_tall_pct- tall alterniflora (associated with low marsh)
                  ...
                  #trees
                  grepl("Conifer|Acer|Catalpa|Betula|C. canadensis|Fagus|Fraxinus|Juglans|Juniperus|Pinus|Platanus|Populus|Prunus|Quercus|Rhus|Rhamnus",.x)&.x!="Angiosperm/Conifer shrub" ~"trees_pct",
                  ...
                  #other less common species that occur in greater than 10 marsh patches. Group by genus.
                  #Lonicera (honeysuckle) species "Lonicera"
                  
                  #juncus sp "Juncus sp|Juncus effusus|Juncus canadensis|Juncus biflorus|Juncus articulatus|Juncus arcticus"
                  #remove algae and non-descript names 
...
rapid_dat6<-rapid_dat5%>%
  mutate(across(c(LowMarshCC,HighMarshCC,SaltMarshTBorderCC,BrackishTBorderCC,InvasivesCC, 
         ...
         across(c(alt_pct,alt_short_pct,alt_tall_pct,patens_pct,phrag_pct,distichlis_pct,gerardii_pct,
                  ...
                  water_pct,upland_pct,trees_pct),
                ~ifelse(is.na(.x),0,.x)),
         ...
         sp_pct_available=ifelse(if_all(c(alt_pct,alt_short_pct,alt_tall_pct,patens_pct,phrag_pct,distichlis_pct,gerardii_pct,
                                          low_marsh_pct,high_marsh_pct,brackish_border_pct,saltmarsh_border_pct,
                                          water_pct,upland_pct,trees_pct),is.zero),0,1)
  )%>%
...
rapid_dat7$dom_species<-colnames(
  ...
  apply(rapid_dat7[,c("alt_pct","alt_short_pct","alt_tall_pct","patens_pct","phrag_pct","distichlis_pct","gerardii_pct",
                      "low_marsh_pct","high_marsh_pct","brackish_border_pct","saltmarsh_border_pct",
                      "water_pct","upland_pct","trees_pct")],1,which.max)
]
...
write.csv(rapid_dat9, paste0(path_out,"Intermediate_outputs/Survey_Vegetation/processed_rapid_veg.csv"),row.names = F)
